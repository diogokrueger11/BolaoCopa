...
fetch("/api/ranking").then(res=>res.json()).then(data=>{
  ...
  body.innerHTML=data.ranking.length?data.ranking.map(row=>`<tr class="ranking-row" data-participant="${row.participantId}" tabindex="0"><td><b>${row.position}</b></td><td><div class="person">${avatar(row.profile)}<span><b>${row.profile.name||"Participante"}</b><small>Ver palpites corretos</small></span></div></td><td>${row.correct}</td><td><strong>${row.points}</strong></td></tr>`).join(""):'<tr><td colspan="4" class="empty">Nenhum participante salvou palpites ainda.</td></tr>';
...
async function showDetails(id){
  ...
  details.innerHTML=`<p class="eyebrow">PALPITES CORRETOS</p><div class="ranking-person">${avatar(data.profile)}<div><h2>${data.profile.name||"Participante"}</h2><span>${data.correct} acertos &bull; ${data.points} pontos</span></div></div><div class="ranking-correct-list">${data.games.length?data.games.map(item=>{const game=gameById[item.gameId];return`<div class="ranking-correct-game"><b>${label(game?.home||item.gameId)} x ${label(game?.away||"")}</b><span>Acertou: ${pickLabel(item.pick,game)}</span></div>`}).join(""):'<div class="empty">Este participante ainda nao possui acertos.</div>'}</div>`;
}
