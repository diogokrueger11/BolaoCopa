...
export function setupNav(active){document.querySelectorAll("[data-page]").forEach(link=>{link.href=withToken(link.getAttribute("href"));link.classList.toggle("active",link.dataset.page===active)});renderUserPill();renderAdminControls()}
export async function renderAdminControls(){if(!token)return;try{const res=await fetch(`/api/access-links?token=${encodeURIComponent(token)}`);if(!res.ok)return;const adminUrl=withToken("/acessos.html"),menu=document.querySelector(".menu");if(menu&&!menu.querySelector("[data-admin]"))menu.insertAdjacentHTML("beforeend",`<a href="${adminUrl}" data-admin>Administração</a>`);if(document.querySelector(".admin-controls"))return;const wrap=document.createElement("div");wrap.className="admin-controls";wrap.innerHTML=`<button id="admin-sync">Sincronizar Bitrix</button><a href="${adminUrl}">Links de acesso</a>`;document.body.appendChild(wrap);wrap.querySelector("#admin-sync").onclick=async event=>{const button=event.currentTarget;button.disabled=true;button.textContent="Sincronizando...";try{const sync=await fetch(`/api/sync-bitrix?token=${encodeURIComponent(token)}`,{method:"POST"});const data=await sync.json();if(!sync.ok)throw new Error(data.error);toast(`${data.employees} funcionários sincronizados.`);setTimeout(()=>location.reload(),900)}catch(error){toast(error.message||"Falha ao sincronizar.");button.disabled=false;button.textContent="Sincronizar Bitrix"}}}catch{}}
